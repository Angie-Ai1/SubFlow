...
with tab_subs:
    ...
    if active_df.empty:
        ...
        edited_active = st.data_editor(
            ...
            column_config={
                ...
                "最近扣款": st.column_config.DatetimeColumn(
                    "最近扣款", disabled=True, format="YYYY-MM-DD"
                ),
        ...
        selected_ids = (
            edited_active[edited_active["選取"]]["id"].tolist()
            if "id" in edited_active.columns
            else []
        )
    ...
    if inactive_df.empty:
        ...
        hide = [
            c
            for c in ["id", "月均費用", "啟用", "服務商", "下次扣款", "訂閱開始"]
            if c in inactive_df.columns
        ]
    ...
    with st.expander("✏️ 編輯 / 刪除訂閱"):
        if subs_df.empty:
            ...
            with st.form("edit_sub_form"):
                ...
                ed_amount = ec4.number_input(
                    "金額 *", value=float(sel["金額"]), min_value=0.0, step=1.0, format="%.2f"
                )
    ...
    if bills_df.empty:
        ...
        if range_sel != st.session_state.bills_range_sel:
            ...
            st.session_state.bills_date_from = (
                data_min if range_sel == "所有" else today - timedelta(days=_range_delta[range_sel])
            )
            st.session_state.bills_date_to = today
        ...
        mask = (pd.to_datetime(bills_df["扣款時間"]).dt.date >= date_from) & (
