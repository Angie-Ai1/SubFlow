...
市面上已有 Gemini 等 AI 助理能夠摘要郵件、回答「我這個月花了多少錢」這類問題。
但在實際使用後，我發現這類工具有幾個結構性盲點：

- **被動查詢，非主動提醒**：要有感覺「快扣款了」才會去問，但訂閱費往往在渾然不覺中從帳戶消失。
- **無持久化記錄**：每次對話都是全新的，沒有跨月比較、沒有歷史趨勢、也沒辦法問「今年哪個月花最多」。
- **依賴自然語言，誤差大**：AI 解析「$12.99 charged」和「已向您收取 NT$399」的準確率不穩定，更無法在解析失敗時給出明確的錯誤原因。
- **缺乏行動介面整合**：我習慣用 LINE 處理日常事務，但 AI 助理無法推播通知到 LINE，只能被動等我去問。

SubFlow 的目標不是取代 AI 助理，而是把「訂閱追蹤」這個特定場景做到自動化、可稽核、有主動行為能力。
